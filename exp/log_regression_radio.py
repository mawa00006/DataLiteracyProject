...
# preprocessing ---------------------------------------------------------------
# exclude the NaNs
data = data.dropna(subset=['Radio Therapy', 'Tumor Size', 'Tumor Stage',
...
chemo = data[data['Radio Therapy'] == 1]
no_chemo = data[data['Radio Therapy'] == 0]

plt.hist(data['Age at Diagnosis'], bins=20, alpha=0.5, label='all')
plt.hist(chemo['Age at Diagnosis'], bins=20, alpha=0.5, label='radio therapy')
...
X_train, X_test, y_train, y_test = train_test_split(data[['Age at Diagnosis']],
                                                    data[['Radio Therapy']], test_size=0.25, random_state=42)
